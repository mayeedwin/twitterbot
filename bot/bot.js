...
const searchQuery = { q: 'from%3Amiddleeasteye%20OR%20from%3Aeyeonpalestine%20OR%20from%3Awolpalestine%20OR%20from%3Aqudsnen%20OR%20from%3Apalyouthmvmt%20OR%20from%3Apalestinercs)%20-filter%3Areplies', count: 50, result_type: 'mixed', include_entities: false };
...
const palestineListId = { list_id:  '1746614491717022134', count: 50, include_rts: false, include_entities: false};
...
const retweetLatest = async() => {
  try {
	...
	//twit.get('lists/statuses', palestineListId, (error, data, response) => {
      twit.get('search/tweets', searchQuery, (error, data) => {
    // If our search request to the server had no errors...
    if (error) {
      // However, if our original search request had an error, we want to print it out here...
      console.log(error.message);
//	console.log(respsonse.message);
    } else {
